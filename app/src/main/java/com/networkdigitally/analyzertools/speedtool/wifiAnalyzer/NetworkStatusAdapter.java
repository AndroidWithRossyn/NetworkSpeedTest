...
//
    ...
    private void setValuesToListView(int position, ViewHolder viewHolder)
    {
        ...
        for (int i = 0; i < this.values.size(); i++)
        {
            ...
            {
                viewHolder.imageView.setImageResource(R.drawable.wireless_0);
            }
            else if (Utility.convertQualityToStepsQuality(quality,5) == 2)
            {
                viewHolder.imageView.setImageResource(R.drawable.wireless_1);
            }
            else if (Utility.convertQualityToStepsQuality(quality,5) == 3)
            {
                viewHolder.imageView.setImageResource(R.drawable.wireless_2);
            }
            else if (Utility.convertQualityToStepsQuality(quality,5) == 4)
            {
                viewHolder.imageView.setImageResource(R.drawable.wireless_3);
            }
            else if (Utility.convertQualityToStepsQuality(quality,5) == 5)
            {
                viewHolder.imageView.setImageResource(R.drawable.wireless_4);
            }
        }
    }
